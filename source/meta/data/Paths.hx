...
class Paths
{
	// Hacky solution for disabling caching in the menu
	public static var forceCaching:Bool = true;
	...
	inline static public function inst(song:String, prefix:String = ""):Null<openfl.media.Sound>
	{
		var songKey:String = '${formatToSongPath(song)}/Inst' + prefix;
	...
	inline static public function image(key:String, ?library:String):FlxGraphic
	{
		...
		// darnell mode! ahahaa
		var returnAsset:FlxGraphic = returnGraphic((ClientPrefs.darnell && FlxG.save.data.KUTValue == 2) ? 'characters/darnell' : key,
	...
	// 			if (!changedAtlasJson)
	// 			{
	// 				if (exists('images/$originalPath/spritemap$st.json'))
	// 				{
	...
	static public function cacheBitmap(file:String, ?bitmap:BitmapData = null, ?allowGPU:Bool = true)
	{
		...
		localTrackedAssets.push(file);
		if (allowGPU && forceCaching && ClientPrefs.gpuCaching)
		{
